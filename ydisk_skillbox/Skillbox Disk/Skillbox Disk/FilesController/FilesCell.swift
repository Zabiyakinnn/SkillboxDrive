...
//  Skillbox Disk
//
//  Created by Дмитрий Забиякин on 01.06.2024.
//
...
class FilesCell: UITableViewCell {
    ...
    func configure(_ viewModel: Item) {
        ...
        if let token = UserDefaults.standard.string(forKey: UserDefaultsKey.saveToken) {
            ...
            switch (viewModel.preview, viewModel.mime_type, viewModel.type) {
            case (nil, "application/zip", "file"):
                imageFile.image = UIImage(named: "zip")
            case(nil, "audio/mpeg", "file"):
                imageFile.image = UIImage(named: "music")
            case(nil, nil, "dir"):
                imageFile.image = UIImage(named: "folder")
            default:
                if let token = UserDefaults.standard.string(forKey: UserDefaultsKey.saveToken) {
                    ...
                    print("отсутсвие токена авторизации")
                }
            }
        }
    }
